...
def _homelab_pad_mimo_qkv(config) -> bool:
    if os.environ.get("HOMELAB_MIMO_FP8_TAIL") != "1" or tuple(config.weight_shape) != (3392, 4096):
        return False
    props = torch.cuda.get_device_properties(torch.cuda.current_device())
    return props.name == "NVIDIA RTX 6000D" and (props.major, props.minor, props.multi_processor_count) == (12, 0, 156)
...
class B12xFp8BlockScaledMMKernel(Fp8BlockScaledMMLinearKernel):
    ...
    def can_implement(
        ...
        if out_features <= 0 or (out_features % 128 != 0 and not _homelab_pad_mimo_qkv(config)):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if _homelab_pad_mimo_qkv(self.config):
            # The final 128-row scale block already covers the 64-row tail.
            # Appending zero weight rows preserves all original logical outputs.
            w = layer.weight
            assert tuple(w.shape) == (3392, 4096)
            padded = torch.zeros((3456, 4096), dtype=w.dtype, device=w.device)
            padded[:3392].copy_(w)
    ...
    def apply_weights(self, layer, x, bias=None, **kwargs):
        ...
        logical_n = self.config.weight_shape[0]
        if output.shape[-1] != logical_n:
            output = output[..., :logical_n].contiguous()
