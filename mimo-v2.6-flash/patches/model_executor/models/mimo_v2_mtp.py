...
class MiMoV2MultiTokenPredictor(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        num_mtp_layers = min(config.num_nextn_predict_layers, spec_cfg.num_speculative_tokens)
        if not 1 <= num_mtp_layers <= 3:
            raise ValueError("MiMo lab supports verified checkpoint layers 0..2 only")
