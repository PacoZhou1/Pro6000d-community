...
class DraftModelSpeculator(BaseSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # MiMo-only proposal calibration. The same adjusted logits are
        # sampled AND cached for rejection; target sampling is unchanged.
        factor = float(os.environ.get("HOMELAB_MIMO_DRAFT_SCALE", "1"))
        assert 0.5 <= factor <= 2.0
        self._homelab_draft_scale = torch.tensor(factor, dtype=torch.float32, device=device)
        self._homelab_scale_path = os.environ.get("HOMELAB_MIMO_DRAFT_SCALE_FILE", "")
        self._homelab_last_scale_poll = 0.0
        self._homelab_scale_value = factor
        self._homelab_mimo_scale_enabled = (
            self.speculative_config.draft_model_config.hf_config.model_type == "mimo_v2_mtp"
            and (factor != 1.0 or bool(self._homelab_scale_path))
        )
    ...
    def sample_draft(
        ...
        if draft_logits is not None:
            logits = self.model.compute_logits(hidden_states)
            if self._homelab_mimo_scale_enabled:
                logits = logits * self._homelab_draft_scale
    ...
    def _copy_request_inputs(
        ...
        if self._homelab_mimo_scale_enabled and self._homelab_scale_path:
            now = time.monotonic()
            if now - self._homelab_last_scale_poll > 1.0:
                self._homelab_last_scale_poll = now
                with open(self._homelab_scale_path) as f:
                    value = float(f.read().strip())
                if not 0.5 <= value <= 2.0:
                    raise ValueError("MiMo draft scale outside qualified range")
                if value != self._homelab_scale_value:
                    self._homelab_draft_scale.fill_(value)
                    self._homelab_scale_value = value
                    logger.info("HOMELAB MiMo proposal logit scale=%s; target unchanged", value)
