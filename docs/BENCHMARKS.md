> 名称说明：“社区原始镜像”指直接使用社区发布的镜像，在我们四张6000D上测出的起点成绩：单路132.09、8路并发366.66、长文本处理8054 tps。下文的另一套硬件称为“4张满血RTX PRO 6000 96GB”。
>
> 首页用约数方便阅读。单路约200取自195–205的优化实测；当前优先长文本速度的常驻配置，单路另测为187.69。各项成绩的具体配置在这里保留，不表示一次测试同时得到所有数字。

# Benchmark：社区原始镜像在本机、本机优化后与4 张满血 RTX PRO 6000 96GB 参考

所有本机速度单位为 tok/s，verifier 为 steps/s。本文只保留原始起点和本机优化后记录；中间试验仍保存在机器可读 JSON 中，不作为这份公开对比的叙事主线。

![社区原始镜像在本机、本机优化后与4 张满血 RTX PRO 6000 96GB 参考](final-comparison.svg)

## 三列对比

| 指标 | 社区原始镜像在本机 | 本机优化后 | 4 张满血 RTX PRO 6000 96GB 参考 |
|---|---:|---:|---:|
...
4 张满血 RTX PRO 6000 96GB 参考为四张 stock RTX PRO 6000 96 GB；本机为四张 RTX 6000D 84 GB、350 W/卡。社区 R30 的 C8/C32/C64 未重测，因此这些行只引用 R28.1；它们不能被解读成 R30 同 profile 的横向测试。

## 详细测试设置

| 行 | 原始与最终本机 profile | 满血 96GB 四卡测试设置 | 不能据此声称什么 |
|---|---|---|---|
